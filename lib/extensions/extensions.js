...
const throwIfBadFilepath = subject => {
  const path = subject.item.path ?? subject.item
  if (('' + path).indexOf('\\') > -1) {
...
const getPublicUrl = config => {
  const itemPath = config.item.path ?? config.item
  if (itemPath.endsWith('assets') && config.packageName === 'govuk-frontend') {
    return '/govuk/assets'
  } else {
    return ['', 'extension-assets', config.packageName]
      .concat(itemPath.split('/').filter(filterOutParentAndEmpty))
...
const getFileSystemPath = config => {
  ...
  return getPathFromProjectRoot('node_modules',
    config.packageName,
    itemPath.split('/').filter(filterOutParentAndEmpty).join(path.sep))
}
