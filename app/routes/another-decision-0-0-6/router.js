...
router.post(`/system-payment-selector-router`, (req, res) => {
  ...
  else if (paymentTheory == 'e2') {
    res.redirect(`cya-payment-breakdown-e2`)
  }
  ...
  else if (paymentTheory == 'w') {
    res.redirect(`cya-payment-breakdown-w`)
  }
  else if (paymentTheory == 's1') {
    res.redirect(`cya-payment-schedule-s1`)
  }
  else if (paymentTheory == 'pp') {
    res.redirect(`pp-breakdown-w`)
  }
   else {
    res.redirect(`XXX`)
  }
...
router.post(`/qppt-m-9m-change-check-router`, (req, res) => {
  const qpptMchangecheck = req.session.data['qppt-m-9m-change-check']
...
router.post(`/payment-recipent-check-router`, (req, res) => {
  ...
  if (obcTL8 == 'Yes') {
    res.redirect(`payment-recipent-applicant`)
  }
  else if (obcTL8 == 'No') {
